...
export default function WallpaperGeneratorScreen({ route, navigation }: any) {
    ...
    const renderPreview = () => {
        switch (mode) {
            ...
            case 'goal':
                /**
                 * ✅ KEY CHANGE:
                 * Replaced GoalDisplay (shows just a number) with YearGrid in goalMode.
                 * This means the wallpaper preview shows the SAME dot grid as the home screen:
                 *   - white  = days before today
                 *   - orange = today
                 *   - grey   = remaining days
                 *   - red    = goal day
                 *
                 * Requires: data.goalDayOfYear (number) — pass this from GoalSetupScreen.
                 */
                return (
                    <View style={{ alignItems: 'center' }}>
                        {data.goalTitle ? (
    ...
    return (
        <View style={styles.container}>
            {/* Hidden capture area — full screen size for wallpaper quality */}
            <ViewShot
                ref={viewShotRef}
                style={[
                    styles.captureArea,
            ...
            <SafeAreaView style={styles.uiContainer}>
                ...
                {/* Live preview visible behind the bottom sheet */}
